import { ContentType, Post, PostComment, VoteType } from "@/interface/global";
  ...
  return useMutation({
    ...
    onMutate: async ({ type, content_id, content_type }) => {
      ...
      if (content_type === "POST") {
        ...
        const previousPostsQueries = queryClient.getQueriesData(postsQueryFilter);
        ...
        return { previousSinglePost, previousPostsQueries, singlePostQueryKey, type: "POST" };
    ...
    onError: (err, newVote, context: any) => {
      ...
      if (context?.type === "POST") {
        ...
        if (context.previousSinglePost && context.singlePostQueryKey) {
          queryClient.setQueryData(context.singlePostQueryKey, context.previousSinglePost);
        }
