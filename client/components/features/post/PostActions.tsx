...
export default function PostActions({ content, type, onOpenComments }: PostActionsProps) {
  ...
  const sections = {
    POST: (
      <View className={`flex-row items-center gap-3 w-full`}>
        <View
          className={`flex-row items-center gap-x-2 py-[2px] px-2 rounded-full border border-gray-200 ${
            isVoteUP ? "bg-bg-yellow" : isVoteDown ? "bg-bg-red" : "bg-bg-gray"
          }`}
        >
          {/* Botón UP */}
          <TouchableOpacity
          ...
          <Text
            className={`text-[14px] font-dosis-bold ${
              isVoteUP || isVoteDown ? "text-white" : "text-text-4"
            }`}
          >
            {totalLikes}
          </Text>
        ...
        <TouchableOpacity
          ...
          className="flex-row items-center gap-1.5 bg-bg-gray py-[6px] px-4 border border-gray-200"
        >
          <Octicons name="comment" size={16} color="#707070" />
          <Text className="text-[14px] font-dosis-bold text-text-4">
    ...
    COMMENT: (
      <View className={`flex-row items-center justify-end gap-x-2 w-16`}>
        {/* Botón UP */}
        <TouchableOpacity
          activeOpacity={0.7}
          onPress={() => handleVoteUp()}
        ...
        <Text
          style={{
            color: isVoteUP ? "#e5a657" : isVoteDown ? "#ef4444" : "#707070",
          }}
          className={`text-[14px] font-dosis-bold`}
        >
          {totalLikes}
  ...
  return <View>{sections[type]}</View>;
}
